...
def write_crpyt_script(dir, to_read, to_write):
    ...
    with open(dir + "/crypt.py", "w") as script:
        ...
        script.write("# in \'result/ZIPFLV\' with the key \'MySuperSecureKey\' and writes\n")
        script.write("# the result into the file \'zipflv.flv\' that will also be located\n")
        script.write("# in \'result/ZIPFLV\'.\n")
...
if __name__ == "__main__":
    ...
    os.system("python mergeFiles.py %s/ZIP/animals.zip %s/PDF/file.pdf %s/pdfzip.pdf" % (data_dir, data_dir, result_dir))
